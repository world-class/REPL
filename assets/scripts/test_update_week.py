...
@given(
    st.datetimes(
        min_value=datetime(2019, 1, 1), max_value=datetime(2021, 10, 9)
    )
)
...
def test_get_current_week_number_returns_1_on_Sunday_right_before_semester():
    """
    We want to see "Week 1" when we're about to start the next semester.
    """
    next_semester_start_date = datetime(2021, 10, 11)  # Monday
    today = datetime(2021, 10, 10)  # Day before starting
...
@given(
    st.datetimes(
        min_value=datetime(2021, 10, 11), max_value=datetime(2021, 10, 17)
    )
)
def test_get_current_week_number_returns_1_on_first_week_of_semester(
    ...
    Any date within the first week should return 1, including on the following
    Sunday.
